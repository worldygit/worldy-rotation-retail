--- AceConfigDialog-3.0 generates WorldyAceGUI-3.0 based windows based on option tables.
-- @class file
-- @name AceConfigDialog-3.0
-- @release $Id: AceConfigDialog-3.0.lua 1296 2022-11-04 18:50:10Z nevcairiel $

local LibStub = LibStub
local gui = LibStub("WorldyAceGUI-3.0")
local reg = LibStub("WorldyAceConfigRegistry-3.0")

local MAJOR, MINOR = "WorldyAceConfigDialog-3.0", 86
...
AceConfigDialog.tooltip = AceConfigDialog.tooltip or CreateFrame("GameTooltip", "WorldyAceConfigDialogTooltip", UIParent, "GameTooltipTemplate")
...
local function CleanUserData(widget, event)
	...
	if widget.type == "WorldyTreeGroup" then
	...
	if widget.type == "WorldyTabGroup" then
		widget:SetTabs(nil)
	...
	if widget.type == "WorldyDropdownGroup" then
			...
			if v.type == "group" then
				if inline or pickfirstset(v.dialogInline,v.guiInline,v.inline, false) then
					...
					if name and name ~= "" then
						GroupContainer = gui:Create("WorldyInlineGroup")
						GroupContainer:SetTitle(name or "")
					else
						GroupContainer = gui:Create("WorldySimpleGroup")
					end
				...
				if v.type == "execute" then
					...
					control = CreateControl(v.dialogControl or v.control, iconControl and "WorldyIcon" or "WorldyButton")
					if iconControl then
						if not width then
				...
				elseif v.type == "input" then
					control = CreateControl(v.dialogControl or v.control, v.multiline and "WorldyMultiLineEditBox" or "WorldyEditBox")
				...
				elseif v.type == "toggle" then
					control = CreateControl(v.dialogControl or v.control, "WorldyCheckBox")
					control:SetLabel(name)
				...
				elseif v.type == "range" then
					control = CreateControl(v.dialogControl or v.control, "WorldySlider")
					control:SetLabel(name)
				...
				elseif v.type == "select" then
					...
					if v.style == "radio" then
						...
						control = gui:Create("WorldyInlineGroup")
						...
						for _, value in ipairs(sorting) do
							...
							local radio = gui:Create("WorldyCheckBox")
							radio:SetLabel(text)
				...
				elseif v.type == "multiselect" then
					...
					if control then
						...
						for s = 1, #valuesort do
							...
							local text = values[value]
							local check = gui:Create("WorldyCheckBox")
							check:SetLabel(text)
				...
				elseif v.type == "color" then
					control = CreateControl(v.dialogControl or v.control, "WorldyColorPicker")
					control:SetLabel(name)
				...
				elseif v.type == "WorldyKeybinding" then
					control = CreateControl(v.dialogControl or v.control, "WorldyKeybinding")
					control:SetLabel(name)
					control:SetKey(GetOptionsMemberValue("get",v, options, path, appName))
					control:SetCallback("OnKeyChanged",ActivateControl)

				elseif v.type == "header" then
					control = CreateControl(v.dialogControl or v.control, "WorldyHeading")
					control:SetText(name)
					control.width = "fill"

				elseif v.type == "description" then
					control = CreateControl(v.dialogControl or v.control, "WorldyLabel")
					control:SetText(name)
...
function AceConfigDialog:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	if (not (hasChildGroups and not inline)) or (grouptype ~= "tab" and grouptype ~= "select" and (parenttype == "tree" and not isRoot)) then
		if container.type ~= "WorldyInlineGroup" and container.type ~= "WorldySimpleGroup" then
			scroll = gui:Create("WorldyScrollFrame")
	...
	if hasChildGroups and not inline then
		...
		if grouptype == "tab" then
			...
			local tab = gui:Create("WorldyTabGroup")
		...
		elseif grouptype == "select" then
			...
			local selectGroup = gui:Create("WorldyDropdownGroup")
		...
		elseif (parenttype ~= "tree") or isRoot then
			local tree = gui:Create("WorldyTreeGroup")
...
function AceConfigDialog:Open(appName, container, ...)
	...
	if type(container) == "table" and container.type == "WorldyBlizOptionsGroup" and #path > 0 then
		for i = 1, #path do
...
function AceConfigDialog:AddToBlizOptions(appName, name, parent, ...)
	...
	if not BlizOptions[appName][key] then
		local group = gui:Create("WorldyBlizOptionsGroup")
